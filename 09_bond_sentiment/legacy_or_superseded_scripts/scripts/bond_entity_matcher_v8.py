...
# -*- coding: utf-8 -*-
"""
Match Telegram/Smart-Lab texts to bond issuers and offerings, with context inheritance.

What the script does
--------------------
1. Loads:
   - panel_data_enriched.xlsx
   - issuer_aliases_candidates.csv
   - issue_aliases_candidates.csv
2. Builds alias dictionaries for issuers and offerings.
3. Matches texts explicitly:
   - Telegram: messages.text
   - Smart-Lab posts: title + tags + text
   - Smart-Lab comments: text
4. Resolves one final entity per document when possible:
   - explicit offering match has priority over explicit issuer match
   - ambiguous documents are kept unresolved
5. Propagates context conservatively:
   - Telegram replies inherit only from a directly matched parent (depth=1 by default)
   - Smart-Lab comments do NOT inherit from post by default; can be enabled explicitly
6. Saves candidate matches + resolved matches back to SQLite.
7. Optionally builds offering-level mention coverage CSV around book_date.

Assumptions
-----------
- data.db (or user-specified Telegram DB) contains table `messages`
- smartlab_bonds.db contains tables `posts` and `comments`
- comments do NOT have parent-comment ids
- sentiment scores are NOT computed here; this script only resolves entity linkage

Recommended workflow
--------------------
1. Run the script.
2. Inspect *_match_candidates and *_matches_resolved tables.
3. Manually review false positives / missing aliases.
4. Improve alias dictionaries.
5. Re-run.
"""
...
# Legacy alias — has_bond_context() still referenced by match_document for
# the "require_context" use_rule on issuer aliases.
BOND_CONTEXT_KEYWORDS = BOND_KEYWORDS
...
# depth 1/2/3 inherited weights
INHERIT_DECAY = {1: 0.70, 2: 0.50, 3: 0.35}
DEFAULT_MAX_DEPTH = 1
...
def iter_unique_ngrams(
    tokens: List[str],
    max_n: int = DEFAULT_MAX_ALIAS_WORDS,
    max_alias_words: Optional[int] = None,
) -> Iterable[Tuple[int, str]]:
    """Yield unique n-grams from longest to shortest.

    max_alias_words is accepted as an alias for backward compatibility with
    older call sites. If provided, it overrides max_n.
    """
    if not tokens:
        return
    if max_alias_words is not None:
        max_n = max_alias_words
    seen = set()
...
def has_bond_context(text_norm: str) -> bool:
    return any(kw in text_norm for kw in BOND_CONTEXT_KEYWORDS)
...
def build_alias_dicts(
    ...
    include_review_issue_aliases: bool = False,
    ...
    if include_review_issue_aliases:
        allowed_issue_rules.add("review")
...
def process_telegram(
    ...
    # Resolve inheritance with memoization.
    sys.setrecursionlimit(1000000)

    def resolve_final(source_key: str, stack: Optional[set] = None) -> Optional[ResolvedEntity]:
        row = meta.get(source_key)
        if row is None:
            return None
        if row["final_entity"] is not None:
            return row["final_entity"]
        if stack is None:
            stack = set()
        ...
        if row["explicit_status"] == "resolved":
            row["final_entity"] = row["explicit_entity"]
            stack.remove(source_key)
            return row["final_entity"]

        if row["explicit_status"] == "ambiguous":
            row["final_entity"] = None
            stack.remove(source_key)
            return None
        ...
        # Conservative inheritance: only direct parent with its own explicit resolved match.
        ...
        # Very short low-information replies are only inherited for offering-level anchors.
        if row["is_short_reply"] and parent_ent.entity_level != "offering":
            row["final_entity"] = None
            stack.remove(source_key)
            return None

        decay = INHERIT_DECAY.get(inherited_depth, 0.25)
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Bond issuer/offering matcher for Telegram and Smart-Lab")
    p.add_argument("--telegram-db", default="data.db", help="Path to Telegram DB (default: data.db)")
    p.add_argument("--smartlab-db", default="smartlab_bonds.db", help="Path to smartlab_bonds.db")
    p.add_argument("--panel", default="panel_final_v2.xlsx", help="Path to panel_data_enriched.xlsx")
    p.add_argument("--issuer-aliases", default="issuer_aliases.xlsx", help="Path to aliases.xlsx")
    p.add_argument("--issue-aliases", default="issue_aliases_candidates_fixed.xlsx", help="Path to issue_aliases_candidates.csv")
    p.add_argument("--chunk-size", type=int, default=DEFAULT_CHUNK_SIZE)
    p.add_argument("--max-depth", type=int, default=DEFAULT_MAX_DEPTH)
    p.add_argument("--max-gap-hours", type=int, default=DEFAULT_MAX_TIME_GAP_HOURS)
    p.add_argument("--window-days", type=int, default=DEFAULT_WINDOW_DAYS)
    p.add_argument("--max-alias-words", type=int, default=DEFAULT_MAX_ALIAS_WORDS)
    p.add_argument("--include-review-issue-aliases", action="store_true", help="Deprecated: review issue aliases are now included by default")
    p.add_argument("--enable-smartlab-comment-inherit", action="store_true", help="Enable conservative inheritance from post to comment on Smart-Lab (off by default)")
    p.add_argument("--skip-coverage", action="store_true", help="Do not build offering-level mention coverage CSV")
    p.add_argument("--coverage-out", default="mention_coverage_window7.csv", help="Output CSV for coverage features")
    return p
...
def main() -> None:
    ...
    issuer_aliases_by_len, issue_aliases_by_len, _ = build_alias_dicts(
        ...
        include_review_issue_aliases=False,
