...
#   Rscript run_biprobit_gjrm_cluster_bootstrap_v3.R --bootstrap --boot-b=399 regression_ready.csv out_boot
...
cluster_bootstrap_gjrm <- function(data, original_fit, B = BOOTSTRAP_B,
  ...
  param_names <- names(original_est)
  ...
  for (b in seq_len(B)) {
    ...
    for (i in seq_along(sampled_clusters)) {
      ...
      # This variable just makes repeated clusters distinguishable if you inspect the data.
    ...
    # If a resample loses outcome variation, GJRM cannot estimate the binary model.
    ...
    est <- tryCatch(
      extract_key_estimates(fit_result$fit),
      error = function(e) {
        add_error(b, "extract", conditionMessage(e), boot_data)
        rep(NA_real_, length(original_est))
      }
    )

    if (is.null(names(est))) names(est) <- param_names
    est <- est[param_names]

    # The earlier version required ALL parameters, including theta, to be finite.
    # That was too strict: if theta parsing failed once, the whole bootstrap draw was counted as failed.
    # Now we keep each finite parameter separately and compute SEs parameter-by-parameter.
    finite <- is.finite(est)
    if (any(finite)) {
      boot_mat[b, finite] <- est[finite]
      extract_ok[b] <- TRUE
    } else {
      add_error(b, "extract", "Fit succeeded but no key estimates could be extracted", boot_data)
    }
  ...
  rows <- lapply(param_names, function(param) {
    vals <- boot_df[[param]]
