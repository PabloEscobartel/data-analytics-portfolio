...
DEPOSIT_NOISE_TOKENS = {
    "депозит", "депозита", "депозите", "депозиту", "депозитом",
    "депозиты", "депозитов", "депозитах", "депозитам",
}

ORGANIZER_KEYWORDS = [
    "организатор", "организаторами", "букраннер", "андеррайтер",
    "агент по размещению", "агентом по размещению", "соорганизатор",
]

VENUE_PREPOSITIONS = {"в", "во", "на", "через", "у", "из"}
BROKER_VENUE_STEMS = {
    "доступ", "появ", "видн", "заявк", "поручен", "налил", "раздал",
    "купить", "купил", "куплю", "купят", "покуп",
    "прода", "продаж", "продать", "продал",
    "счет", "счёт", "квик", "quik", "приложен", "витрин",
    "терминал", "комисси", "предлага", "проход", "пройти", "квест",
    "письм", "бумажн", "приш", "присл", "зачисл", "получ",
}
...
DUAL_ROLE_ISSUERS = {
    ...
    "ВБРР", "РОСБАНК", "МСП Банк", "РН Банк",
    "Металлинвестбанк", "ХКФ Банк",
}
...
PRIMARY_ACTION_STEMS = {
    "размест", "размеща", "провод", "провед", "открыл", "откры", "собира",
    "установ", "начнет", "выпуст", "готовит", "объяв",
    "выкуп", "погас", "плат", "выплат", "исполн", "сниз", "повыс",
}
ORGANIZER_ROLE_STEMS = {"организатор", "букраннер", "андеррайтер", "соорганизатор", "агентом"}
ORGANIZER_ROLE_TOKENS = {"агент", "агента", "агенты"}
BANK_CONTEXT_STEMS = {"банк", "банков", "брокер", "инвестиц", "заявк", "счет", "счету"}
DIGEST_PARENT_KEYWORDS = {
    "дайджест", "обзор дня", "календар", "новости эмитентов",
    "утренние новости", "вечерние новости", "события рынка",
    "план по первич", "статистика рынка",
}

RANKED_RESOLUTION_MIN_MARGIN = 0.12
...
class TopicFlags:
    ...
    has_organizer_context: bool = False
    ...
    organizer_keywords_found: List[str] = field(default_factory=list)
...
def classify_topic(text_norm: str) -> TopicFlags:
    ...
    deposit_hits = sorted(set(tokenize(text_norm)) & DEPOSIT_NOISE_TOKENS)
    if deposit_hits:
        flags.has_broker_noise = True
        flags.noise_keywords_found.extend(deposit_hits)

    for kw in ORGANIZER_KEYWORDS:
        if normalize_text(kw) in text_norm:
            flags.has_organizer_context = True
            flags.organizer_keywords_found.append(kw)

    return flags


def _has_stem(tokens: Iterable[str], stems: Iterable[str]) -> bool:
    stems_tuple = tuple(stems)
    return any(tok.startswith(stems_tuple) for tok in tokens)


def span_has_primary_action_evidence(tokens: List[str], start: int, end: int) -> bool:
    """Whether an alias span sits near verbs that make it the acting issuer."""
    left = max(0, start - 4)
    right = min(len(tokens), end + 8)
    return _has_stem(tokens[left:right], PRIMARY_ACTION_STEMS)


def span_has_organizer_role_evidence(tokens: List[str], start: int, end: int) -> bool:
    """Whether an alias span appears in an organizer/agent list."""
    left = max(0, start - 8)
    right = min(len(tokens), end + 4)
    window = tokens[left:right]
    return bool(set(window) & ORGANIZER_ROLE_TOKENS) or _has_stem(window, ORGANIZER_ROLE_STEMS)


def span_has_broker_venue_evidence(tokens: List[str], start: int, end: int) -> bool:
    """Whether a bank alias is used as broker/app venue: "доступно в ВТБ"."""
    left = max(0, start - 6)
    right = min(len(tokens), end + 8)
    window = tokens[left:right]
    has_venue_preposition = (
        (start > 0 and tokens[start - 1] in VENUE_PREPOSITIONS)
        or (end < len(tokens) and tokens[end] in VENUE_PREPOSITIONS)
    )
    has_service_action_after = _has_stem(tokens[end:right], BROKER_VENUE_STEMS)
    return (has_venue_preposition or has_service_action_after) and _has_stem(window, BROKER_VENUE_STEMS)


def issuer_alias_allowed(meta: Dict[str, Any], tokens: List[str], start: int, end: int, topic: TopicFlags) -> bool:
    """Special precision guards for aliases that are common financial words."""
    issuer = str(meta.get("issuer", ""))
    alias_norm = str(meta.get("alias_norm", ""))

    # "альфа" is often generic alpha/excess-return jargon.  Full aliases like
    # "альфа-банк" still match via longer n-grams before this guard matters.
    if issuer == "Альфа-Банк" and alias_norm == "альфа":
        left = max(0, start - 3)
        right = min(len(tokens), end + 4)
        return topic.has_broker_noise or _has_stem(tokens[left:right], BANK_CONTEXT_STEMS)

    return True


def is_broad_inheritance_parent(
    raw_text: str,
    text_norm: str,
    n_candidate_issuers: int,
    n_candidate_offerings: int,
) -> bool:
    """Block inheritance from digest/list posts where one anchor is unreliable."""
    if any(kw in text_norm for kw in DIGEST_PARENT_KEYWORDS):
        return True

    n_candidates = n_candidate_issuers + n_candidate_offerings
    if n_candidates >= 3:
        return True

    # Bulleted market calendars often contain one resolved entity plus many
    # unmatched instruments; replies to them should not inherit a single anchor.
    bulletish = raw_text.count("\n") + raw_text.count("•") + raw_text.count("🔹") + raw_text.count("👉")
    return bulletish >= 8 and n_candidates >= 2
...
def should_suppress_dual_role_match(
    ...
    """Drop only obvious non-bond bank/broker false positives.

    Ambiguous semantic roles such as organizer/agent/broker venue are kept as
    resolved potential matches and marked in resolution_note. The sentiment
    classifier should decide final target relevance.
    """
    ...
    # Offering-level explicit evidence is strong enough to keep.
    if entity.entity_level == "offering":
        return False

    # Strong issue evidence for the same issuer is strong enough to keep.
    ...
    # No bond-market context: usually app/service/stock/banking chatter.
    if not topic.has_bond_context:
        return True
    ...
    *different* position in the text, it still matches correctly.
    ...
    for n in range(max_n, 0, -1):
        ...
        for i in range(num_tokens - n + 1):
            ...
            matched_any = False
            has_primary_action = span_has_primary_action_evidence(tokens, i, i + n)
            has_organizer_role = span_has_organizer_role_evidence(tokens, i, i + n)
            ...
            for meta in issuer_metas:
                if meta["use_rule"] == "require_context" and not context_flag:
                    continue
                if not issuer_alias_allowed(meta, tokens, i, i + n, topic):
                    continue
                matched_any = True
                issuer = meta["issuer"]
                hit = issuer_hits.setdefault(
                    issuer,
                    {
                        ...
                        "first_token": i,
                        "has_primary_action_evidence": False,
                        "has_organizer_role_evidence": False,
                        "has_broker_venue_evidence": False,
                    },
                )
                hit["first_token"] = min(hit.get("first_token", i), i)
                if has_primary_action:
                    hit["has_primary_action_evidence"] = True
                if has_organizer_role:
                    hit["has_organizer_role_evidence"] = True
                if issuer in DUAL_ROLE_ISSUERS and span_has_broker_venue_evidence(tokens, i, i + n):
                    hit["has_broker_venue_evidence"] = True
                if str(meta.get("risk_flag", "low")).lower() == "low":
                    ...
                    meta_with_span = dict(meta)
                    meta_with_span["span_start"] = i
                    meta_with_span["span_end"] = i + n
                    meta_with_span["has_primary_action_evidence"] = has_primary_action
                    meta_with_span["has_organizer_role_evidence"] = has_organizer_role
                    meta_with_span["has_broker_venue_evidence"] = (
                        issuer in DUAL_ROLE_ISSUERS
                        and span_has_broker_venue_evidence(tokens, i, i + n)
                    )
                    tentative_nonlow_by_issuer[issuer].append(meta_with_span)
            ...
            for meta in issue_metas:
                ...
                meta_with_span["has_primary_action_evidence"] = has_primary_action
                meta_with_span["has_organizer_role_evidence"] = has_organizer_role
                pending_issue_rows.append(meta_with_span)

            if matched_any:
                consumed.update(span)

    for meta in pending_issue_rows:
        issuer_present = meta["issuer"] in issuer_hits
        issuer_hit_for_issue = issuer_hits.get(meta["issuer"], {})
        issuer_is_organizer_only = (
            bool(issuer_hit_for_issue.get("has_organizer_role_evidence"))
            and not bool(issuer_hit_for_issue.get("has_primary_action_evidence"))
        )
        # Unsafe short codes require issuer presence in the same text.
        if meta["use_rule"] == "require_issuer_or_window" and not issuer_present:
            continue
        is_strong_issue = issue_alias_is_strong(meta["alias_type"])
        is_code_only = issue_alias_is_code_only(meta["alias_type"])
        ...
        hit = issue_hits.setdefault(
            oid,
            {
                ...
                "first_token": meta.get("span_start", 10**9),
                "has_primary_action_evidence": False,
                "has_organizer_role_evidence": False,
            },
        )
        ...
        hit["alias_types"].add(meta["alias_type"])
        hit["max_score"] = max(hit["max_score"], meta["base_score"])
        hit["first_token"] = min(hit.get("first_token", meta.get("span_start", 10**9)), meta.get("span_start", 10**9))
        if meta.get("has_primary_action_evidence"):
            hit["has_primary_action_evidence"] = True
        if meta.get("has_organizer_role_evidence"):
            hit["has_organizer_role_evidence"] = True
        if is_strong_issue:
            hit["has_strong_issue_evidence"] = True
            hit["eligible_for_resolution"] = True
        if is_code_only:
            hit["has_code_only_issue_evidence"] = True
            if issuer_present and not issuer_is_organizer_only:
                hit["eligible_for_resolution"] = True
        if not is_code_only:
            hit["eligible_for_resolution"] = True
    ...
    for issuer, metas in tentative_nonlow_by_issuer.items():
        hit = issuer_hits.setdefault(
            issuer,
            {
                ...
                "max_score": 0.0,
                "has_low_risk_alias": False,
                "first_token": min(m.get("span_start", 10**9) for m in metas),
                "has_primary_action_evidence": False,
                "has_organizer_role_evidence": False,
                "has_broker_venue_evidence": False,
            },
        ...
        for meta in metas:
            if verify_nonlow_issuer_alias(
                ...
                if meta.get("has_organizer_role_evidence"):
                    hit["has_organizer_role_evidence"] = True
                if meta.get("has_broker_venue_evidence"):
                    hit["has_broker_venue_evidence"] = True
    ...
    issuer_hits = {
        issuer: {
            ...
            "first_token": hit.get("first_token", 10**9),
            "has_primary_action_evidence": bool(hit.get("has_primary_action_evidence")),
            "has_organizer_role_evidence": bool(hit.get("has_organizer_role_evidence")),
            "has_broker_venue_evidence": bool(hit.get("has_broker_venue_evidence")),
            "has_low_risk_alias": bool(hit.get("has_low_risk_alias")),
        }
...
def _position_bonus(first_token: int) -> float:
    if first_token <= 3:
        return 0.12
    if first_token <= 10:
        return 0.08
...
def _score_issue_hit(hit: Dict[str, Any], issuer_hits: Dict[str, Dict[str, Any]], topic: TopicFlags) -> float:
    score = float(hit.get("max_score") or 0.0)
    score += _position_bonus(int(hit.get("first_token", 10**9)))
    if hit.get("has_strong_issue_evidence"):
        score += 0.25
    if hit.get("issuer") in issuer_hits:
        score += 0.12
    if hit.get("has_primary_action_evidence"):
        score += 0.10
    if topic.has_primary_context:
        score += 0.03
    return score


def _score_issuer_hit(hit: Dict[str, Any], topic: TopicFlags) -> float:
    score = float(hit.get("max_score") or 0.0)
    score += _position_bonus(int(hit.get("first_token", 10**9)))
    if hit.get("has_primary_action_evidence"):
        score += 0.25
    if topic.has_bond_context:
        score += 0.02
    if topic.has_primary_context:
        score += 0.03

    issuer = hit.get("issuer")
    if issuer in DUAL_ROLE_ISSUERS and topic.has_broker_noise and not topic.has_primary_context:
        score -= 0.25
    if issuer in DUAL_ROLE_ISSUERS and hit.get("has_organizer_role_evidence") and not hit.get("has_primary_action_evidence"):
        score -= 0.35
    if issuer in DUAL_ROLE_ISSUERS and hit.get("has_broker_venue_evidence") and not hit.get("has_primary_action_evidence"):
        score -= 0.35
    return score


def _ranked_winner(scored_hits: List[Tuple[float, Dict[str, Any]]]) -> Tuple[Optional[Dict[str, Any]], float, float]:
    if not scored_hits:
        return None, 0.0, 0.0
    scored_hits = sorted(scored_hits, key=lambda x: x[0], reverse=True)
    top_score, top_hit = scored_hits[0]
    second_score = scored_hits[1][0] if len(scored_hits) > 1 else float("-inf")
    margin = top_score - second_score
    if len(scored_hits) == 1 or margin >= RANKED_RESOLUTION_MIN_MARGIN:
        return top_hit, top_score, margin
    return None, top_score, margin


def _issuer_resolution_note(prefix: str, hit: Dict[str, Any]) -> str:
    note = prefix
    if hit.get("has_primary_action_evidence"):
        note += "_primary_action"
    if hit.get("has_organizer_role_evidence") and not hit.get("has_primary_action_evidence"):
        note += "_organizer_role"
    if hit.get("has_broker_venue_evidence") and not hit.get("has_primary_action_evidence"):
        note += "_broker_venue"
    return note


def _make_issue_entity(hit: Dict[str, Any], note: str, score: Optional[float] = None) -> ResolvedEntity:
    return ResolvedEntity(
        ...
        match_score=round(float(score if score is not None else hit["max_score"]), 4),
        resolution_note=note,
    )


def _make_issuer_entity(hit: Dict[str, Any], note: str, score: Optional[float] = None) -> ResolvedEntity:
    return ResolvedEntity(
        issuer=hit["issuer"],
        entity_level="issuer",
        match_origin="explicit",
...
def resolve_explicit_entity(
    issuer_hits: Dict[str, Dict[str, Any]],
    issue_hits: Dict[int, Dict[str, Any]],
    topic: Optional[TopicFlags] = None,
) -> Tuple[str, Optional[ResolvedEntity], str]:
    """
    Returns: (status, resolved_entity, note)
      status in {'resolved','ambiguous','none'}

    v9 originally treated every 2+ candidate case as ambiguous.  We keep that
    conservative fallback, but first allow a clear ranked winner when the text
    gives positional/role evidence (e.g. primary issuer first, banks later as
    organizers or comparison names).
    """
    topic = topic or TopicFlags()

    eligible_issue_hits = [
        hit for hit in issue_hits.values()
        if hit.get("eligible_for_resolution", True)
    ]
    if eligible_issue_hits:
        if len(eligible_issue_hits) == 1:
            hit = eligible_issue_hits[0]
            note = "unique_issue_match"
            if hit.get("has_code_only_issue_evidence") and not hit.get("has_strong_issue_evidence"):
                note = "unique_issue_match_code_with_issuer"
            return "resolved", _make_issue_entity(hit, note), note

        scored = [(_score_issue_hit(hit, issuer_hits, topic), hit) for hit in eligible_issue_hits]
        winner, score, margin = _ranked_winner(scored)
        if winner is not None:
            note = f"ranked_issue_match_margin_{margin:.3f}"
            return "resolved", _make_issue_entity(winner, note), note
        return "ambiguous", None, f"ambiguous_issue_matches:{len(eligible_issue_hits)}_low_margin:{margin:.3f}"

    if issue_hits and not eligible_issue_hits:
        return "none", None, "issue_candidates_not_eligible"

    if issuer_hits:
        if len(issuer_hits) == 1:
            hit = next(iter(issuer_hits.values()))
            note = _issuer_resolution_note("unique_issuer_match", hit)
            return "resolved", _make_issuer_entity(hit, note), note

        scored = [(_score_issuer_hit(hit, topic), hit) for hit in issuer_hits.values()]
        winner, score, margin = _ranked_winner(scored)
        if winner is not None:
            note = _issuer_resolution_note(f"ranked_issuer_match_margin_{margin:.3f}", winner)
            return "resolved", _make_issuer_entity(winner, note), note
        return "ambiguous", None, f"ambiguous_issuer_matches:{len(issuer_hits)}_low_margin:{margin:.3f}"
...
def process_telegram(
    ...
    for chunk in iterator:
        for _, row in chunk.iterrows():
            ...
            explicit_status, explicit_entity, note = resolve_explicit_entity(issuer_hits, issue_hits, topic)
            ...
            meta[source_key] = {
                ...
                "n_candidate_issuers": len(issuer_hits),
                "n_candidate_offerings": len(issue_hits),
                "is_broad_parent": is_broad_inheritance_parent(
                    text,
                    normalize_text(text),
                    len(issuer_hits),
                    len(issue_hits),
    ...
    def resolve_final(source_key: str) -> Optional[ResolvedEntity]:
        ...
        if parent_row.get("is_broad_parent"):
            return None

        if parent_ent.entity_level == "issuer":
            if gap is not None and gap > min(max_gap_hours, 24):
                return None
            if parent_row.get("n_candidate_issuers", 0) + parent_row.get("n_candidate_offerings", 0) > 2:
                return None
            if not row["bond_context"] and not parent_row["primary_context"]:
                return None
...
def process_smartlab(
    ...
    for chunk in iterator2:
        for _, row in chunk.iterrows():
            ...
            explicit_status, explicit_entity, note = resolve_explicit_entity(issuer_hits, issue_hits, comment_topic)
