...
class FocalLoss(torch.nn.Module):
    ...
    Especially useful for sentiment where positive/negative are often
    confused with neutral. gamma=2.0 means easy examples (high confidence)
    contribute ~25x less to loss than hard examples.
    """
...
def train_classifier(
    ...
    if task == "sentiment":
        loss_fn = FocalLoss(weight=class_weights, gamma=2.0, label_smoothing=label_smoothing)
        print(f"  Loss: FocalLoss (gamma=2.0)", flush=True)
    else:
        loss_fn = torch.nn.CrossEntropyLoss(weight=class_weights, label_smoothing=label_smoothing)
        print(f"  Loss: CrossEntropyLoss", flush=True)
...
def train_all(
    ...
    # Sentiment uses lower LR (finer distinction) and more epochs
    sent_lr = lr * 0.75  # 1.5e-5 if base lr is 2e-5
    sent_epochs = epochs + 2  # 8 if base epochs is 6
    print(f"  Sentiment LR: {sent_lr:.1e} (base × 0.75)")
    print(f"  Sentiment epochs: {sent_epochs} (base + 2)")
    train_classifier(
        sent_df,
        task="sentiment",
        label_col="sentiment",
...
def predict_all(input_csv: str, output_csv: str, batch_size: int = 64) -> None:
    ...
    df["sentiment_score"] = 0.0  # continuous: P(positive) - P(negative)
    df["p_negative"] = 0.0
    df["p_neutral"] = 0.0
    df["p_positive"] = 0.0
    ...
    for i in range(0, len(relevant_idx), batch_size):
        ...
        # Get raw probabilities for continuous score
        encodings = sent_tok(
            batch_issuers, batch_texts,
            max_length=sent_max_len, padding=True,
            truncation="only_second", return_tensors="pt",
            return_token_type_ids=True,
        )
        inputs = {k: v.to(DEVICE) for k, v in encodings.items()}
        with torch.no_grad():
            outputs = model_forward(sent_model, inputs)
            probs = torch.softmax(outputs.logits, dim=-1).cpu().numpy()

        for j, idx in enumerate(batch_idx):
            df.at[idx, "sentiment_pred"] = labels[j]
            df.at[idx, "sentiment_conf"] = round(float(confs[j]), 4)
            df.at[idx, "p_negative"] = round(float(probs[j, SENTIMENT_LABELS["negative"]]), 4)
            df.at[idx, "p_neutral"] = round(float(probs[j, SENTIMENT_LABELS["neutral"]]), 4)
            df.at[idx, "p_positive"] = round(float(probs[j, SENTIMENT_LABELS["positive"]]), 4)
            df.at[idx, "sentiment_score"] = round(
                float(probs[j, SENTIMENT_LABELS["positive"]] - probs[j, SENTIMENT_LABELS["negative"]]), 4
            )
    ...
    if relevant_mask.any():
        rel_df = df.loc[relevant_mask]
        print(f"  Continuous score: mean={rel_df['sentiment_score'].mean():.4f}  "
              f"std={rel_df['sentiment_score'].std():.4f}  "
              f"range=[{rel_df['sentiment_score'].min():.4f}, {rel_df['sentiment_score'].max():.4f}]")
