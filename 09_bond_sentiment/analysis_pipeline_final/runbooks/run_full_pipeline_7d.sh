...
RS="${RSCRIPT:-Rscript}"
BOOT_B="${BOOT_B:-999}"
DATASET_TEMPLATE="regression_dataset_lookback7_{window}_{min_messages}/regression_ready.csv"
...
echo "1) Build 7-day regression datasets from panel_final_v3.xlsx"
for WINDOW in book_open book_close; do
  for MIN_MSG in 3 5 10; do
    "${PY}" "${PIPELINE_DIR}/scripts/build_pipeline.py" \
      ...
      --output-dir "regression_dataset_lookback7_${WINDOW}_${MIN_MSG}"
  done
done
...
"${PY}" "${PIPELINE_DIR}/scripts/run_descriptive_statistics.py" \
  --output-xlsx descriptive_statistics_lookback7_all_windows.xlsx \
  --output-plots distribution_plots_lookback7_all_windows.pdf \
  --dataset-template "${DATASET_TEMPLATE}"
...
"${PY}" "${PIPELINE_DIR}/scripts/run_prediction_batch.py" \
  ...
  --output-dir prediction_results_batch_lookback7 \
  --windows book_open \
  --min-messages 3 5 10 \
  --dataset-template "${DATASET_TEMPLATE}"
...
"${PY}" "${PIPELINE_DIR}/scripts/run_regressions_high_spread.py" \
  regression_dataset_lookback7_book_open_5/regression_ready.csv \
...
"${RS}" "${PIPELINE_DIR}/scripts/run_biprobit_gjrm_cluster_bootstrap_v4.R" \
  ...
  --all-samples \
  --dataset-template="${DATASET_TEMPLATE}" \
  gjrm_biprobit_bootstrap_lookback7

echo "8) Combine GJRM bootstrap SE files"
"${PY}" "${PIPELINE_DIR}/scripts/combine_gjrm_bootstrap_se.py" \
  --all-samples \
  gjrm_biprobit_bootstrap_lookback7 \
  gjrm_biprobit_bootstrap_se_combined_lookback7_all_samples.csv \
  --xlsx gjrm_biprobit_bootstrap_se_combined_lookback7_all_samples.xlsx

echo "Done: 7-day pipeline."
