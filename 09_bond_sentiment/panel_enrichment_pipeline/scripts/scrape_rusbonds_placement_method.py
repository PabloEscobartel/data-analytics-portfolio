...
AUTH_FILE = "rusbonds_auth.json"


def load_rusbonds_auth():
    token = os.getenv("RUSBONDS_BEARER_TOKEN", "").strip()
    cookies = {}

    cookies_json = os.getenv("RUSBONDS_COOKIES_JSON", "").strip()
    if cookies_json:
        cookies = json.loads(cookies_json)
    elif os.path.exists(AUTH_FILE):
        with open(AUTH_FILE, encoding="utf-8") as f:
            auth = json.load(f)
        token = token or str(auth.get("bearer_token", "")).strip()
        cookies = auth.get("cookies", {}) or {}

    if token:
        cookies.setdefault("auth._token.local", f"Bearer%20{token}")
    return token, cookies


BEARER_TOKEN, COOKIES = load_rusbonds_auth()
